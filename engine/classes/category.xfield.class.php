		...
		foreach( $xfields as $xfname => $xfattr ) {
			if ( $xfattr[1] == "text" ) {
				...
				for ( $x = 0; $x < count( $_opt[0] ); $x++ ) {
					$_val = ( trim( $xfielddata[ $xfname ] ) == trim( $_opt[0][$x] ) ) ? " checked" : "";
					$_options .= "<input class=\"xfr\" type=\"radio\"{$_val} value=\"{$_opt[0][$x]}\" name=\"catxf[{$xfname}]\">&nbsp;{$_opt[1][$x]}&nbsp;&nbsp;";
				}
				...
				for ( $x = 0; $x < count( $_opt[0] ); $x++ ) {
					$_val = ( trim( $xfielddata[ $xfname ] ) == trim( $_opt[0][$x] ) ) ? " selected" : "";
					$_options .= "<option value=\"{$_opt[0][$x]}\"{$_val}>{$_opt[1][$x]}</option>";
				}
				$_options .= "</select>";
				$_result .= "<div class=\"form-group\"><label class=\"control-label col-lg-2\">{$xfattr[2]} ({$xfattr[0]})</label><div class=\"col-lg-10\">{$_options}</div></div>";
